...
<div class="container grid px-6 mx-auto">
    <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">
        {{ isset($guruMapel) ? 'Edit' : 'Tambah' }} Guru Mapel
    </h2>

    <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
        <form action="{{ isset($guruMapel) ? route('admin.guru-mapel.update', $guruMapel->id) : route('admin.guru-mapel.store') }}" method="POST">
            @csrf
            @if(isset($guruMapel))
                @method('PUT')
            @endif
            ...
            <label class="block mt-4 text-sm">
                ...
                >
                    ...
                    <option value="{{ $mapel->id }}" {{ old('mapel_id', $data->mapel_id) == $mapel->id ? 'selected' : '' }}>
                        {{ $mapel->nama }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="flex justify-end mt-6">
                ...
                >
                    {{ isset($guruMapel) ? 'Update' : 'Simpan' }} Guru Mapel
                </button>
            </div>
        </form>
    </div>
